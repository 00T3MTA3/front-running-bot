...
const HTTP_RPC = "https://crimson-weathered-wildflower.bsc.discover.quiknode.pro/3b20b117ac6d0ce51168066a8d329fc926a7dea7/";
const WSS = "wss://prettiest-chaotic-road.bsc.discover.quiknode.pro/343ca333a4a630c483e1747f8968cb2a65b28ff9/";

const TARGETTOKEN = null;//'0x55d398326f99059fF775485246999027B3197955';//null; // put address of token you just want to front run.  null for any token
...
const SELL_SLIPPAGE = 1;//25;// how much less off the BNB you are willing to not get when you sell tokens
...
const BUY_GASLIMIT_BOOST = 3;//2;// This is a multiplier to multiply the gaslimit... also helps to jackup the gas price

const BLACKLIST_TOKEN = '0xB82BEb6Ee0063Abd5fC8E544c852237aA62CBb14';//null;//'0x55d398326f99059fF775485246999027B3197955';// BUSD - we dont want this
//--------------------------
//--------------------------
...
const buyToken = async(account,tokenContract,gasLimit,gasPrice, swapFunc, outMin)=>{
  ...
  if (parseInt(slippage) !== 0) {
    ...
    amountOutMin = amounts[1].sub(amounts[1].div(1000).mul(`${slippage}`));
  }
  ...
  if(!TEST){
    ...
    if(swapFunc == 1)
    {
      ...
      //const amounts = await router(account).getAmountsOut(amountIn, [BNB_CONTRACT, tokenContract]);
      console.log("120    swapping : swapExactETHForTokens: outMin:", ethers.utils.formatUnits(outMin,'ether'));
      try{
    ...
    {
      console.log("135    swapfunc : swapExactETHForTokensSupportingFeeOnTransferTokens: outMin:", ethers.utils.formatUnits(outMin,'ether'));
      try{
    ...
    else if(swapFunc == 3)//swapETHForExactTokens
    {
      ...
      const amounts = await router(account).getAmountsOut(amountIn, [BNB_CONTRACT, tokenContract]);
      console.log("152    swapfunc : swapETHForExactTokens: outMin:", ethers.utils.formatUnits(outMin,'ether'));
      try{
        tx = await router(account).swapETHForExactTokens(
          amounts[1].sub(amounts[1].mul(5).div(1000)),
          [BNB_CONTRACT, tokenContract],
          account.address,
          (Math.round(Date.now() / 1000) + (60 * 10)),//(Date.now() + 1000 * 60 * 10),
          {
    ...
    console.log("166:blocknumber : ",blockNumber, 'block delta:', (blockNumber-startBlockNumber));
    ...
    console.log("171: tx received: blockNumber:",receipt.blockNumber, 'block delta:', (receipt.blockNumber-startBlockNumber));
...
const sellToken = async(account,tokenContract,gasLimit,gasPrice,value=999)=>{
  ...
  const amountIn = tokenBalance;//tokenBalance.mul(value).div(1000)
  ...
  if (parseInt(slippage) !== 0) {
    amountOutMin = amounts[1].sub(amounts[1].mul(`${slippage}`).div(1000));
  } else {
    amountOutMin = amounts[1].sub(amounts[1].mul(10).div(1000));//amounts[1]
  }
  ...
  if(!TEST){
    ...
    console.log("205: approve finished:", receipt_approve.transactionHash, "blockNumber:",receipt_approve.blockNumber, "block delta:", (receipt_approve.blockNumber-startBlockNumber), "gas used:",receipt_approve.gasUsed.toString(), "cumalGas:", receipt_approve.cumulativeGasUsed.toString(), "effectGas:", receipt_approve.effectiveGasPrice, "block:", receipt_approve.blockNumber, "status:",receipt_approve.status)
    if (receipt_approve && receipt_approve.blockNumber && receipt_approve.status === 1) { 
      ...
      console.log("219: wait on tx : ",swap_txn.hash);
      console.log("219: blockNumber: ",swap_txn.blockNumber);
      console.log("220:      value : ",ethers.utils.formatUnits(swap_txn.value));
      console.log("221:   gasprice : ",ethers.utils.formatUnits(swap_txn.gasPrice,'gwei'));
      console.log("222:   gasLimit : ",swap_txn.gasLimit.toString());
      processing = false;//dont wait start the next one
      const receipt = await swap_txn.wait();
      console.log("220: tx finished",tokenContract, "blockNumber:", receipt.blockNumber)
      if (receipt && receipt.blockNumber && receipt.status === 1) { // 0 - failed, 1 - success
        console.log(`Transaction https://bscscan.com/tx/${receipt.transactionHash} mined, status success`);
      } else if (receipt && receipt.blockNumber && receipt.status === 0) {
        console.log(`Transaction https://bscscan.com/tx/${receipt.transactionHash} mined, status failed`);
...
var init = async function () {
  ...
  //let url = "https://crimson-weathered-wildflower.bsc.discover.quiknode.pro/3b20b117ac6d0ce51168066a8d329fc926a7dea7/";
  customHttpProvider = new ethers.providers.JsonRpcProvider(HTTP_RPC);//url);
  const account = wallet.connect(customHttpProvider)

  //const account = wallet.connect(customWsProvider)
  const iface = new ethers.utils.Interface(['function swapExactETHForTokens(uint amountOutMin, address[] calldata path, address to, uint deadline)',
    ...
    if(value>WHALE_SIZE) {//nachomod 10
      ...
      if(result.length>0){
        ...
        if(result[1].length>0){
          ...
          startBlockNumber = await customHttpProvider.getBlockNumber();
    
          console.log("------294:token : (",tokenAddress, startBlockNumber,") -------------------");
          if(BLACKLIST_TOKEN !== null && tokenAddress === BLACKLIST_TOKEN)
          {
            processing = false;
            return;
          }
          ...
          if(TARGETTOKEN !== null && tokenAddress !== TARGETTOKEN){
            processing = false;
            return;
          }
          ...
          console.log("         outMin : ", ethers.utils.formatUnits(outMin, 'ether') );
          ...
          await buyToken(account,tokenAddress,transaction.gasLimit,buyGasPrice, swapFunc, outMin);//transaction.gasLimit
